...
local_resource(
  ...
  ignore=['frontend/node_modules', 'frontend/dist', 'frontend/.vite']
)
...
local_resource(
  ...
  serve_cmd='''
    cd backend && \
    DATABASE_URL="postgres://tally:password@localhost:5433/tally?sslmode=disable" \
    SUPABASE_URL="https://aagdtgsmjpmiqtdbdquj.supabase.co" \
    PORT="8081" \
    ./bin/server
  ''',
  deps=['backend'],
